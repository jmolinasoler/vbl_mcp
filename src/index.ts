#!/usr/bin/env node
/**
 * MCP server for the Basketball Vlaanderen (VBL) public API
 * (vblcb.wisseq.eu — the backend used by basketbal.vlaanderen).
 *
 * All data is read-only. GUIDs may contain spaces (e.g. team GUIDs like
 * "BVBL1004HSE  2"), which are URL-encoded automatically.
 */
...
const server = new McpServer({ name: "vbl-mcp", version: "0.1.0" });
...
server.registerTool(
  "get_club",
  {
    ...
    description:
      "Get details of one club by its GUID (e.g. BVBL1004), including all its teams and the " +
      "poules (competitions/series) each team plays in. Team GUIDs and poule GUIDs returned " +
      "here can be used with get_team, get_team_matches and get_poule_matches.",
    inputSchema: {
  ...
  async ({ club_guid, search }) =>
    run(async () => {
      ...
      return trimmed.filter((m) =>
        `${m.vnaam} ${m.naam}`.toLowerCase().includes(q)
      );
    })
);
